...
%                 'orientation', 'vi', 'precision', 0, 'expand', 'dataSet', 'total', 'h', 'fontSize', 'tiny');
        ...
        for k=1 :length(classes)
            ...
            config = expExpose(config, 'l', 'mask', {classes{k} 1 2 [8 9 5] [3] [1] 2 1}, 'step', 3, ...
                'highlight', 0, 'highlightColor', -1, 'highlightStyle', 'Best', 'obs', [2], 'percent', 2, ...
                'orientation', 'vi', 'precision', 1, 'expand', 'dataSet', 'noObservation', 1, 'orderSetting', [2 3 1], 'mergeDisplay', 'h', 'save', ['methods' classesNames{k}], 'fontSize', 'small');
...
%         system(['rsync -r ' config.reportPath(1:end-1) ' ~/Dropbox/kAverages/Paper/']);
    ...
    case 'check'
        ...
        for k=1 %:length(classes)
            config = expExpose(config, 't', 'mask', {classes{k} 1 2 [8 9 5] [3] [1] 2 1}, 'step', 3, ...
                'highlight', 0, 'highlightColor', 0, 'highlightStyle', 'best', 'obs', [2], 'percent', 2, ...
                'precision', 1);
        end
