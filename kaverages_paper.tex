...
\begin{enumerate}
\item kkmeans (critere et algo)
\item kaverages (critere et algo)
\item complexity analysis (cpu and memory access)
\item differences qualitatives (critere)
\item difference quantitatives (algo)
\end{enumerate}
}
...
\begin{figure}
\label{fig:moved}
\includegraphics[scale=0.6]{nbMoved.png} 
...
Time series, even though represented as vectors and therefore suitable for any kinds of norm-based clustering, is better compared with elastic measures \cite{Ding:2008:QMT:1454159.1454226}. The Dynamic Time Warping (DTW) measure is a widely used elastic measure in many areas since its introduction for spoken word detection \cite{1163055} and has been hardly challenged for time series mining \cite{conf/kdd/BerndtC94, Rakthanmanon:2013:ABD:2513092.2500489}.

Effective clustering of time series using the DTW measure require similarity based algorithms such as the k-Average algorithm. With some acre, kernel based algorithm can also be considered provided that the resulting similarity matrix is converted into a kernel, \textit{i.e.} the matrix is a Gram matrix \cite{Lanckriet:2004:LKM:1005332.1005334}.

\subsection{Datasets}

A large collection of 43 time series datasets created by many laboratories all over the world and compiled by Prof. Keogh (\url{www.cs.ucr.edu/~eamonn/time_series_data}) is considered.
