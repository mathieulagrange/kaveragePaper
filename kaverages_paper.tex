...
As soon as conceptual data objects can be described using some sort of vocabulary, many kind of computational metrics can be considered to measure the kind of relationships those objects may share. Then, a common need is to group those $n$ objects into $K$ clusters, so that the similarity between objects within the same cluster is maximized. \ml{We shall focus for the purpose of this study on cluster analysis based on iterative divisive hard clustering algorithms. expand or move after}
...
In an effort to minimize explicitly the average intra class similarity, we introduce in this paper a new hard clustering algorithm based on a connectivity model that has the following properties: 
\begin{enumerate}
