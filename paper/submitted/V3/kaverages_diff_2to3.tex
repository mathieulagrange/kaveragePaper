...
%DIF DEL kaverages_paperV2.tex   Wed Apr 18 18:30:48 2018
%DIF ADD kaverages_paper.tex     Tue Apr 24 09:50:55 2018
%DIF < 
%DIF -------
	...
	\While {$L$ is modified} {
		\For {$n \leftarrow 1$ to $N$} {
			\For {$c \leftarrow 1$ to $C$} {
				Compute $Y_{cn}$ following eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:yki} \label{algline:kkmeans_cplx1}
...
\DIFdelbegin \[
\DIFdel{\mathcal{Q}\left(c\right) = \frac{1}{N_c} \sum_{i=1}^{n_c} s\left(o_i, c\right)
}\]
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
	\DIFadd{\mathcal{Q}\left(c\right) = \frac{1}{N_c} \sum_{i=1}^{n_c} s\left(o_i, c\right)
...
\DIFdelbegin \[
\DIFdel{O_2 = \frac{1}{N} \sum_{i=1}^{C} N_i \mathcal{Q}(c_i)
}\]
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
  \DIFadd{\mathcal{O} = \frac{1}{N} \sum_{i=1}^{C} N_i \mathcal{Q}(c_i)
...
\begin{algorithm}
	...
	\textbf{Initialization:}
		...
		Compute initial object-class similarities $S$ following eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:soc_inclass} or eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:soc_notinclass}\;
	\BlankLine
	\While {$L$ is modified} {
...
Considering a class $c$, let us develop the expression of $\mathcal{Q}(c)$ into a more useful form. Since all objects are in $c$, we use the formula in (\DIFaddbegin \DIFadd{eq.~}\DIFaddend \ref{eq:soc_inclass}) to get:
...
\DIFaddbegin \label{sec:delta_definition}
\DIFaddend 
...
by that in (\ref{deltaAdd}); in addition, one object $o$ moves from a class whose quality is $\mathcal{Q}(c_s)$ to one whose quality is $\mathcal{Q}\left(c_t \cup o\right)$, as expressed by \DIFdelbegin \DIFdel{equation}\DIFdelend \DIFaddbegin \DIFadd{eq.}\DIFaddend ~\ref{eq:newQual_add}, which leads to an impact of moving object $o$ from class $c_s$ to class $c_t$ wich can be computed as follows:
...
\DIFaddbegin \begin{algorithm}[h!]
	...
	\KwResult{label vector $L$ defining a partition of the objects into $C$ classes}
	\BlankLine
	\textbf{\DIFadd{Initialization:}}
		\DIFadd{Fill L with random values in $[1..C]$\;
	...
	\While {\DIFadd{$L$ is modified}} {
		\For {\DIFadd{$i \leftarrow 1$ to $N$}} {
			\DIFadd{previousClass $\leftarrow L_i$\;
			nextClass $\leftarrow \mathrm{argmax}_k\,\delta_i(\mathrm{previousClass}, k)$ }\label{algline:kaverages_search}
			\DIFadd{(following the definition of $\delta$ in eq.~\ref{eq:impact_classnorm})\; %DIF >  eq.~\ref{eq:impact_objnorm} or
			}\If {\DIFadd{nextClass $\ne$ previousClass}} {
				\DIFadd{$L_i \leftarrow \mathrm{nextClass}$\;
...
%DIFDELCMD < 	%DIFDELCMD < \label{algo:kaverages}%%%
%DIFDELCMD < 	\SetAlgoLined
%DIFDELCMD < 	\KwData{number of objects $N$, number of classes $C$, similarity matrix $\mathcal{S}$}
%DIFDELCMD < 	\KwResult{label vector $L$ defining a partition of the objects into $C$ classes}
%DIFDELCMD < 	\BlankLine
%DIFDELCMD < 	%%%
\textbf{\DIFdel{Initialization:}}
		%DIFAUXCMD
\DIFdel{Fill L with random values in $[1..C]$\;
		Compute initial object-class similarities $S$ following eq~\ref{eq:soc_inclass} or eq~\ref{eq:soc_notinclass}\;
		Compute initial class qualities }\DIFdelend \DIFaddbegin \section{\DIFadd{Convergence}}
\label{sec:convergence}

\DIFadd{The kernel k-means algorithm ensures convergence if the similarity matrix is semi-definite positive. The k-averages algorithm relaxes this constraint by only requiring symmetricity of the similarity matrix to ensure convergence.
}

\DIFadd{An algorithm is guaranteed to converge if its successive iterations can be tied to a strictly monotonous and bounded quantity. For the k-averages algorithm, this quantity is the objective function itself, as we now show.
}

\DIFadd{Thanks to the rewriting of the class quality function done in eq.~\ref{eq:classQuality}, which only requires the similarity matrix to be symmetrical, we can directly define the allocation decision criterion $\delta$ (eq.~\ref{eq:impact_classnorm}) to be the change in the global objective function $\mathcal{O}$ (eq.~\ref{eq:objective_function}) implied by reallocating an object to a new class. It follows that, as long as reallocations are only performed when $\delta > 0$, $\mathcal{O}$ is strictly increasing throughout the execution of the algorithm.
}

\DIFadd{Moreover, $\mathcal{O}$, defined as a weighted average of the average intra-class similarities for the produced clusters, can be proven to never exceed the maximal similarity between two objects.
}

\DIFadd{Indeed, the average similarity of an object to other members of its class, expressed as $s(o,c)$ in eq.~\ref{eq:soc_notinclass}, is an average of similarities, and therefore lower than or equal to their maximum value. Similarly, the quality }\DIFaddend $\mathcal{Q}$ \DIFdelbegin \DIFdel{following eq~\ref{eq:classQuality}\;
	}%DIFDELCMD < \BlankLine
%DIFDELCMD < 	\While {%%%
\DIFdel{$L$ is modified}%DIFDELCMD < } {
%DIFDELCMD < 		\For {%%%
\DIFdel{$i \leftarrow 1$ to $N$}%DIFDELCMD < } {
%DIFDELCMD < 			%%%
\DIFdel{previousClass $\leftarrow L_i$\;
			nextClass $\leftarrow \mathrm{argmin}_k\,\delta_i(\mathrm{previousClass}, k)$ }%DIFDELCMD < \label{algline:kaverages_search}
%DIFDELCMD < 			%%%
\DIFdel{(following the definition of $\delta$ in \ref{eq:impact_classnorm})\; %DIF <  eq~\ref{eq:impact_objnorm} or
			}%DIFDELCMD < \If {%%%
\DIFdel{nextClass $\ne$ previousClass}%DIFDELCMD < } {
%DIFDELCMD < 				%%%
\DIFdel{$L_i \leftarrow \mathrm{nextClass}$\;
				Update $\mathcal{Q}_\mathrm{previousClass}$ following eq~\ref{eq:newQual_remove}\;
				Update $\mathcal{Q}_\mathrm{nextClass}$ following eq~\ref{eq:newQual_add}\;
				}%DIFDELCMD < \For {%%%
\DIFdel{$j \leftarrow 1$ to $N$}%DIFDELCMD < }{
%DIFDELCMD < 					%%%
\DIFdel{Update $S(j,nextClass)$ and $S(j,previousClass)$ }%DIFDELCMD < \label{algline:kaverages_recompute} \\ %%%
\DIFdel{following eq~\ref{eq:newSimilNewC}\;
				}%DIFDELCMD < }
%DIFDELCMD < 			}
%DIFDELCMD < 		}
%DIFDELCMD < 	}
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{The K-averages algorithm.}}
%DIFAUXCMD
%DIFDELCMD < \end{algorithm}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{of a class, defined as the average of object to class similarities (eq.~\ref{eq:class_qual_base}), is inferior to the maximum value of $s(o,c)$, and therefore to the maximum similarity between objects. Finally, $\mathcal{O}$, a weighted average of $\mathcal{Q}$ values, is, once again, inferior to their maximum value.
}\DIFaddend 

\DIFaddbegin \DIFadd{The objective function $\mathcal{O}$ is thus upper-bounded, and increases at each iteration of the outer loop of Algorithm~\ref{algo:kaverages}; which guarantees its convergence.
}

\DIFaddend \section{Complexity analysis}
...
\begin{algorithm}
	...
	\While {$L$ is modified} {
	    \For {$c \leftarrow 1$ to $C$} {
	        Compute $M_c$ following eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:mc} \label{algline:kkmeans_imp_mc}
	    }
		\For {$n \leftarrow 1$ to $N$} {
			\For {$c \leftarrow 1$ to $C$} {
				Compute $Y_{cn}$ following eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:yki_improved} \label{algline:kkmeans_imp_cplx1}
				(note: $z_{cn} = (L_n == c)\,?\,1\;:\;0$)
			}
			$L_n = \textrm{argmin}_c (Y_{cn})$\;
		}
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{The authors }\DIFaddend would like to acknowledge support for this project
